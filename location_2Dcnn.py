...
batch_size = 32
save = True
filename = '/scratch/users/kcoppess/results/loc2D'

conv_w_shapes = [ [5, 5, 4, c], \
        [5, 5, c, c], \
        [5, 5, c, c], \
        [5, 5, c, c], \
        [2, 2, c, c], \
...
print 'loading data...'
all_inputs = np.loadtxt('/scratch/users/kcoppess/processed/small/twolocation_features.csv', delimiter=',') #pd.read_csv('pairmap2D.csv', delimiter=',', header=None, nrows=Ntrain)
