...
def setup_conv_layers(x_input, w_shapes, b_shapes, N):
    x = tf.reshape(x_input, [-1, columns, rows, 1])
    weights = []
...
class CNN():
    ...
    def construction(self, conv_w_shapes, conv_b_shapes, fc_w_shapes, fc_b_shapes, learning_rate):
        self.pairmap = tf.placeholder(tf.float32, shape=[None, rows*columns], name='pairmap')
    ...
    def save(self, filename, suffix=''):
        saver = tf.train.Saver(tf.global_variables())
        saver.save(self.sess, '%s/%s%s' % (filename, self.name, suffix), global_step=self.i)
